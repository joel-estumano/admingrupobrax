...
export class StockClass extends Core {
  ...
  public headers = ['Código', 'Produto', 'Categoria', 'Qtd', 'Status', 'Motivo', 'Observação', 'Vendedor', 'Comprador', 'Horário'];
  public data;
  public show;
    ...
    data.forEach((e, i) => {
      result.push({
        info: [
          ...
          e.seller ? e.seller : '',
          e.name ? e.name : '',
          new Date(e.created_at?.seconds * 1000).toLocaleString('pt-BR') || new Date(e.createdAt?.seconds * 1000).toLocaleString('pt-BR') || '',
        ],
