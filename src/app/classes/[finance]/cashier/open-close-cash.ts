...
export class OpenOrCloseCashClass extends Core {
    ...
    public data;
    public show;
    public headers = ['Ação', 'Data da Ação', 'Valor', 'Descrição', 'Status', 'Responsável', 'Perfil'];
    override cachePath = 'OpenOrCloseCash';
    override path = 'OpenOrCloseCash';

    constructor(public override getter: HelperGetterService, private pagination: PaginationService, private user: UserClass) {
        super(getter);
        ...
        data.forEach((e, i) => {
            result.push({
                info: [
                    ...
                    e.responsible?.levelName || this.user.getLevelName(e.responsible?.level),
                ],
                actions: [],
                index: i,
                extra: [],
