...
@Injectable()
export class CommissionTypesClass extends Core {
  override cachePath = 'CommissionTypes';
  override path = 'CommissionTypes';
  data: any = [];
  show: any = [];
  pages: any = [];
  currentPage = 0;

  public headers = ['Código', 'Título', 'Categoria', 'Criado em', 'Ação'];
  constructor(
    ...
    data.forEach((e, i) => {
      ...
      result.push({
        info: [
          e.id,
          e.title,
          e.category === 'product' ? 'Por Produto' : 'Por Serviço',
