...
export class Core {
  ...
  */
  ...
  setClassById<T>(id, update = false, cachePath?, collection?): Promise<T> {
    return new Promise((resolve, reject) => {
      this.getter.cache
        .get(cachePath ? cachePath : this.cachePath)
        .then((cache) => {
          if (!cache || update) {
            this.getter.crud
              .get(collection ? collection : this.collection, id)
              .then((http) => {
                this.getter.cache.set(
                  cachePath ? cachePath : this.cachePath,
                  http
                );
                this.set(http);
  ...
  setClassAll<T>(
    ...
    return new Promise((resolve, reject) => {
      this.getter.cache
        ...
        .then((cache) => {
          if (!cache || update) {
            this.getAllHttp(collection ? collection : this.collection)
              .then((res) => {
                this.getter.cache.set(
                  cachePath ? cachePath : this.cachePath,
                  res
