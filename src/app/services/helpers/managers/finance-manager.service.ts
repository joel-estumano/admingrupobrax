      ...
      this.data.forEach((element) => {
        const a = element.filter((e) => {
          if (e.extra[0] === 'movements') {
            if (!activeOption) return e;
            if (activeOption.name == 'Cx loja' && e.extra[1] == 'CX LOJA')
              return e;
            if (
              activeOption.name == 'Cx financeiro' &&
              e.extra[1] == 'CX FINANCEIRO'
            )
              return e;
            if (activeOption.name == 'Cofre' && e.extra[1] == 'COFRE') return e;
            if (activeOption.name == 'Bancos' && e.extra[1] == 'BANCOS')
              return e;
            if (activeOption.name == 'Cheques' && e.extra[1] == 'CHEQUES')
              return e;
          }
        });
        this.show.push(...a);
